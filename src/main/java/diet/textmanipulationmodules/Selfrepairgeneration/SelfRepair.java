   ...
   public Hashtable turnVersions = new Hashtable();
    ...
    private String[] getFirstPart_SecondPart_Summary(Participant p){
       ...
       String summary ="Summary of internal workings:\n";
        ...
        String textTypedAfterADeleteBurst= null;
        ...
        if(textTypedAfterADeleteBurst == null){
            return new String[]{null,null, summary+ "EXITING...CAN'T FIND ANY TEXT THAT WAS TYPED AFTER A DELETE BURST!\n"};
        }
        ...
        Random r = new Random();
        vlongestCandidateTurn = (String)vCandidatesForLongest.elementAt(r.nextInt(vCandidatesForLongest.size()));
        ...
        for(int i=indexOfLongest+1;i<v.size();i++){
              String s = (String)v.elementAt(i);
       ...
       boolean didParticipantDeleteMoreThanTwoCharacters = false;
        if(vlongestCandidateTurn.length()-mostLeftmostDELETE.length() >1){
            didParticipantDeleteMoreThanTwoCharacters=true;
        }
        if(!didParticipantDeleteMoreThanTwoCharacters){
            summary = summary + "EXITING...DIDN'T DELETE MORE THAN TWO CHARACTERS: LENGTH"+ (vlongestCandidateTurn.length()-mostLeftmostDELETE.length())+"  \n";
       ...
       */
       ...
       String  leftmostdeleteSTRIPPED =  StringOperations.stripEnd(mostLeftmostDELETE, null);
       String lastVersionOfTurnSTRIPPED = StringOperations.stripEnd(lastVersionOfTurn, null);
       if(leftmostdeleteSTRIPPED.equalsIgnoreCase(lastVersionOfTurnSTRIPPED)) textToTheRightOfLeftmostDeleteIsAllSpaces = true;
       if(textToTheRightOfLeftmostDeleteIsAllSpaces){
           summary = summary +"EXITING because there was no difference between the leftmost delete turn and the final turn (ignoring trailing whitespace)\n";
          
           return new String[]{null,null, summary};
       }       
       
       
      // vlongestCandidateTurn;
      // mostLeftmostDELETE
      // lastVersionOfTurn 
