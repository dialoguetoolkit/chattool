...
public class IsTypingOrNotTypingOrDeleting extends Thread{
    ...
    public synchronized void processDELETINGByClient(Participant p){
        String mode = (String)htMode.getObject(p);
        this.htLastTyping.putObject(p, new Date().getTime());
        if(mode.equalsIgnoreCase(constants_IsDeleting)) return;
    ...
    private synchronized void informISDELETING(Participant p){  
        ...
        for(int i=0;i<v.size();i++){
            Participant pRecipient = (Participant)v.elementAt(i);
            cC.getC().changeClientInterface_changeBorderOnChatFrame(pRecipient, 6, Color.RED);
    ...
    private synchronized void examineInactivity(){
        ...
        while(eee.hasMoreElements()){
           ...
           // System.err.println("EXAMINING ACTIVITY - 2 "+p.getUsername()+ "inactivity: "+inactivity+" inactivitythreshold"+inactivitythreshold+"  mode:"+mode);
            if(inactivity>inactivitythreshold & (mode.equalsIgnoreCase(this.constants_IsTyping)||mode.equalsIgnoreCase(this.constants_IsDeleting) )){
