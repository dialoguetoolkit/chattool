 ...
 * @author gj
 */
public class FaceComms2016_TURNBYTURN_DYADIC_NDJ extends TurnByTurn_Dyadic{
    ...
    long durationOfFirstStage = 30 * 60 * 1000;
   
    
    public FaceComms2016_TURNBYTURN_DYADIC_NDJ(Conversation c) {
        super(c);
        ...
        if(doLongIsTyping){
            itnt.setInactivityThreshold(90000);
        }
        else{
    ...
    public synchronized void processChatText(Participant sender, MessageChatTextFromClient mct) {
        ...
        if(timeTillStartTyping>0){            
             Conversation.printWSln("Other", "DEBUGINDEX= "+timeTillStartTyping+" mins");
            
        }else{
            Conversation.printWSln("Other", "DEBUGNOW!");
